...
class DBSP:
    ...
    def init(cls, uart, is_debug=False):
        ...
        # 10ms执行依次
        cls.timer = threading.Timer(cls.TIMER_PERIOD_S, cls.update)
        cls.timer.start()
...
class MotionEvent(Event):
    ...
    def tick(self, update_interval_ms):
        '''计时自增'''
        self.cur_time += update_interval_ms

    def is_done(self):
        '''判断事件是否执行完毕'''
        return self.cur_time >= self.delay_ms
    ...
    def send_request_bytes(self):
        ...
        if self.REQUEST_CODE is not None:
            packet_bytes = self.generate_request_bytes()
            # print("Send Data")
            # DBSPUtil.print_bytes_as_hex(packet_bytes)
            DBSP.uart.write(packet_bytes)
...
class ActionEvent(MotionEvent):
    ...
    def __init__(self, action_id, interval):
        ...
        self.interval = interval # Action周期人为指定
        delay_ms = interval
        super().__init__(delay_ms)
...
class MarcoEvent(MotionEvent):
    ...
    def __init__(self, marco_id, interval):
        ...
        self.interval = interval # Marco周期人为指定
        delay_ms = interval
        super().__init__(delay_ms)
